...
const ServiceDetail = ({ title, description, image, index }) => {
    ...
    return (
        <Row as={Grid} row even={isEven} isFirst={isFirst}>
            <Grid item xs={12} sm={6} md={isEven ? { width: 6, push: 1 } : { width: 6 }} flex justifyContent={isFirst ? "flex-start" : "center"} flexDirection="column" mb={isFirst && 10} my={isFirst ? 0 : 10}>
                <Heading level={3} textTransform="uppercase">{title}</Heading>
                <Paragraph>
                    {description}
                </Paragraph>
            </Grid>
            <Grid item xs={12} sm={6} md={ isEven ? { width: 5 } : { width: 5, push: 1 }}>
                <Image
                    src={image}
                    objectFit
                    height="600px"
