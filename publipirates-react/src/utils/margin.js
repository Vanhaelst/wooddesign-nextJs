...
const getMargin = ({ m, mb, ml, mr, mt, mx, my }) => {
    ...
    return css`
        ...
        ${props => marginRight && `margin-right: ${props.theme.spaces[marginRight]}`};
        ${props => marginBottom && `margin-bottom: ${props.theme.spaces[marginBottom]}`};
        ${props => marginLeft && `margin-left: ${props.theme.spaces[marginLeft]}`};
    `
}
